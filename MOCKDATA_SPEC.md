...
You are working in `~/hermes-dispatcher` on branch `feat/live-backend-wiring` (already
...
- Heatmap: render `ov.heatRows`. If `heatRows` is empty, hide the entire heatmap card (don't render
  ...
  or drop the stat).
- **System Monitor**: remove the MS01/MM01 machine switcher entirely (single host).
...
2. Restart live server:
   `kill $(lsof -ti :8787) 2>/dev/null; cd ~/hermes-dispatcher && HERMES_HOME=~/.hermes nohup .venv/bin/uvicorn server:app --host 0.0.0.0 --port 8787 >/tmp/uvicorn.log 2>&1 &`
   then `sleep 3`.
...
4. Grep the built/source to confirm no fixtures remain:
   `grep -rn "rvc-runner\|atlas-etl\|npc-builder\|ops-bot\|w-okada-01\|MS01" app/src` must
