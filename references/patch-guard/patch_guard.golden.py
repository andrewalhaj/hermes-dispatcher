...
# 2h. Cold-store schema guard — validates every write to the LanceDB cold store
# (knowledge.py store() and store_contextualized()) before tbl.add([row]).
...
# 7. Knowledge-DB venv packages (lancedb, pylance, pandas, etc.).
# These are NOT shipped with the Hermes venv — they were installed separately
# and get wiped any time `hermes update` rebuilds the venv. Proven 2026-06-12
# (migration from hil-1 to andrew-Macmini: rsync doesn't transfer venv packages,
# and hermes update nukes them). Check for the sentinel (lancedb) and reinstall
# the full set if missing.
def _heal_knowledge_db_packages():
    import subprocess
    ...
    SENTINEL = "lancedb"
    # Quick sentinel check first
    r = subprocess.run(
        [VENV_PYTHON, "-c", f"import {SENTINEL}"],
        capture_output=True, text=True
    )
    if r.returncode == 0:
        return  # healthy
    # Sentinel missing — reinstall all
    uv = "/root/.local/bin/uv"
    if not os.path.exists(uv):
        problems.append("knowledge-db packages: lancedb missing but uv not found at /root/.local/bin/uv — manual install needed")
        return
    cmd = [uv, "pip", "install", "--python", VENV_PYTHON] + REQUIRED
    r = subprocess.run(cmd, capture_output=True, text=True, timeout=600)
    if r.returncode != 0:
        problems.append(f"knowledge-db packages: uv install failed: {r.stderr.strip()[:200]}")
        return
    # Verify sentinel now importable
    r2 = subprocess.run([VENV_PYTHON, "-c", f"import {SENTINEL}"], capture_output=True, text=True)
    if r2.returncode != 0:
        problems.append("knowledge-db packages: installed but import still fails — investigate")
        return
    actions.append(f"knowledge-db packages: {SENTINEL} was missing; reinstalled {len(REQUIRED)} packages into venv.")


_heal_knowledge_db_packages()
