...
// One Linear issue returned by /api/linear/search.
...
// Linear priority colour by numeric level (0 none, 1 urgent … 4 low).
function linearPriorityColor(priority: number): string {
  switch (priority) {
    case 1: return '#f87171' // urgent
    case 2: return '#fb923c' // high
    case 3: return '#fbbf24' // medium
    case 4: return '#9298ab' // low
    default: return '#565d72' // none
  }
}
...
// One routed-Linear-issue report from /api/linear-reports/messages.
interface LinearOutputMsg {
  role: string
  content: string
  created_at: number
  title: string
...
// Linear icon for the Linear Reports channel (inline SVG, same convention).
function LinearIcon({ size = 15, color = 'currentColor' }: { size?: number; color?: string }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" style={{ flex: 'none' }}>
      <path d="M3 13a9 9 0 0 0 8 8" />
      <path d="M3 9a13 13 0 0 0 12 12" />
      <path d="M4 5.5A17 17 0 0 0 18.5 20" />
      <path d="M9 3.5A19 19 0 0 1 20.5 15" />
    </svg>
  )
}
...
function ChatSidebar({ accent, liveAgents, cronJobs, activeAgent, activeCron, activeSentry, activeLinear, swarmOpen, unreadCounts, collapsed, onToggleSwarm, onSelectAgent, onSelectCron, onSelectSentry, onSelectLinear, onToggleCollapsed }: ChatSidebarProps) {
  ...
  const showLinear = !q || 'linear reports'.includes(q) || 'linear'.includes(q)
  ...
  return (
    ...
    >
      ...
      {/* CHANNELS */}
      {!collapsed && (showCron || showSentry || showLinear) && <GroupHeader label="Channels" />}
      {showCron && (() => {
      ...
      {showLinear && (() => {
        const selected = activeLinear === 'linear'
        const linearColor = '#5e6ad2'
        const linearAvatar = (
          <span style={{ position: 'relative', width: collapsed ? 40 : 44, height: collapsed ? 40 : 44, flex: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: '50%', background: `color-mix(in oklab, ${linearColor} 16%, transparent)`, border: `1px solid color-mix(in oklab, ${linearColor} 42%, transparent)`, color: linearColor }}>
            <LinearIcon size={collapsed ? 20 : 22} color={linearColor} />
          </span>
        )
        if (collapsed) {
          return railRow('linear', selected, onSelectLinear, linearAvatar, 'Linear Reports', unreadCounts['linear'] || 0)
        }
        const now = new Date()
        const timeStr = now.toLocaleTimeString([], { hour: 'numeric', minute: '2-digit', hour12: true })
        return (
          <div
            ...
            title="Routed Linear issues dispatched to the coder fleet"
            style={{
              display: 'flex', alignItems: 'center', gap: 12,
              padding: '10px 12px 10px 13px', cursor: 'pointer',
              background: selected ? 'rgba(246,183,60,0.12)' : 'transparent',
          ...
          >
            ...
            <span style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: 2 }}>
              <span style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 6 }}>
                <span style={{ fontSize: 14, fontWeight: 700, color: selected ? '#f7e9c6' : '#e4e6ee', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', flex: 1 }}>Linear Reports</span>
                <span style={{ fontSize: 11, color: '#565d72', flex: 'none' }}>{timeStr}</span>
              </span>
              <span style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 6 }}>
                <span style={{ fontSize: 12, color: '#9298ab', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', flex: 1 }}>Routed issues</span>
                {(unreadCounts['linear'] || 0) > 0 && (
                  <span style={{
                    minWidth: 20, height: 20, borderRadius: 10,
                    background: '#3b82f6', color: '#fff',
                    fontSize: 11, fontWeight: 700,
...
export default function Chat({ accent, isActive, onUnreadChange }: ChatProps) {
  ...
  // When the Linear Reports channel is selected: 'linear'; else null.
  const [activeLinear, setActiveLinear] = useState<string | null>(null)
  const [linearOutput, setLinearOutput] = useState<LinearOutputMsg[]>([])
  const [linearLoadingCh, setLinearLoadingCh] = useState(false)
  ...
  // /linear search results — interactive panel above the composer.
  // Arrow keys move selection, Enter opens the selected issue in Linear.
  ...
  function openLinearIssue(idx: number) {
    const issue = linearResults[idx]
    if (issue?.url) window.open(issue.url, '_blank', 'noopener,noreferrer')
  }

  async function runLinearSearch(query: string) {
    setLinearOpen(true)
    ...
    try {
      ...
      if (!res.ok) {
        let detail = `Linear search failed (${res.status})`
        try { const j = await res.json(); if (j?.detail) detail = j.detail } catch { /* ignore */ }
        setLinearError(detail)
        return
      }
      const data: { issues: LinearIssue[] } = await res.json()
      setLinearResults(data.issues || [])
      setLinearIdx(0)
      if ((data.issues || []).length === 0) setLinearError(`No Linear issues match "${query}"`)
    } catch {
      setLinearError('Could not reach the Linear search service.')
    } finally {
  ...
  const agent = activeSentry
    ...
    : activeLinear
      ? { name: 'Linear Reports', role: 'Read-only · routed issues', color: '#5e6ad2', icon: '◭' }
    : activeCron
  ...
  const baseThread = activeLinear
    ? (linearLoadingCh
        ? [{ id: 'linear-loading', role: 'agent' as const, text: 'Loading Linear reports…', at: '' }]
        : linearOutput.length === 0
          ? [{ id: 'linear-empty', role: 'agent' as const, text: '(no routed Linear issues yet)', at: '' }]
          : linearOutput.map((m, i) => ({
              id: `linear-${i}`,
              role: 'agent' as const,
              text: `**${m.title}**\n\nPriority ${m.priority} · ${m.coder}` + (m.issue_url ? `\n\n${m.issue_url}` : ''),
  ...
  useEffect(() => {
    if (activeAgent === 'default' || activeCron || activeSentry || activeLinear) return
    let cancelled = false
  ...
  async function selectLinear() {
    setActiveLinear('linear')
    setActiveCron(null)
    setActiveSentry(null)
    setViewSession(null)
    setLinearOutput([])
    setLinearLoadingCh(true)
    setUnreadCounts((prev) => (prev['linear'] ? { ...prev, linear: 0 } : prev))
    try {
      const res = await fetch('/api/linear-reports/messages')
      const data = await res.json() as LinearOutputMsg[]
      setLinearOutput(Array.isArray(data) ? data : [])
    } catch {
      setLinearOutput([])
    } finally {
      setLinearLoadingCh(false)
    }
  }
  ...
  async function send() {
    ...
    // /linear search <query> — interactive issue search, handled client-side.
    const linearMatch = /^\/linear\s+search\s+(.+)$/i.exec(t)
    if (linearMatch) {
      const query = linearMatch[1].trim()
      setDraft('')
      if (query) runLinearSearch(query)
      return
    }
    ...
    // /linear create <title> [p:<priority>] [d:<description>]
    // Creates a Linear issue via the backend, which auto-routes to Kanban.
    if (t.toLowerCase().startsWith('/linear')) {
      const rest = t.slice('/linear'.length).trim()
      const m = rest.match(/^create\b\s*(.*)$/is)
      const k = activeAgent
      const pushAgent = (text: string) =>
        setThreads((s) => ({ ...s, [k]: [...(s[k] || []), { id: 'a' + Date.now(), role: 'agent', text, at: nowTime() }] }))
      // Echo the user's command into the thread.
      setThreads((s) => ({ ...s, [k]: [...(s[k] || []), { id: 'u' + Date.now(), role: 'user', text: t, at: nowTime() }] }))
      setDraft('')
      if (!m) {
        pushAgent('Usage: /linear create <title> [p:<0-4>] [d:<description>]')
        return
      }
      ...
      setRunning(true)
      try {
        const res = await fetch('/api/chat/linear', {
          method: 'POST',
        ...
        if (data.ok) {
          ...
          pushAgent(`${data.confirm}${link}`)
        } else {
          pushAgent(`Linear: ${data.error || 'could not create issue.'}`)
        }
      } catch {
        pushAgent('Linear: could not reach server.')
      } finally {
        setRunning(false)
      }
      return
    ...
    if (k === 'default') {
      setDraft('')
      // Optimistically show the user message in viewSession (single source of truth)
      if (viewSession) {
        const optimisticUser: Message = { id: 'u' + Date.now(), role: 'user', text: displayText, at }
        setViewSession((s) => s ? { ...s, msgs: [...s.msgs, optimisticUser] } : s)
      } else {
        // No active session yet — fall back to the thread so the bubble still shows.
      ...
      try {
          ...
          for (const chunk of parts) {
            ...
            if (ev.type === 'delta') {
              ...
              if (agentMsgId === null) {
                const id = 'a' + Date.now()
                agentMsgId = id
                setViewSession((s) => s ? { ...s, msgs: [...s.msgs, { id, role: 'agent', text: ev.text, at: nowTime() }] } : s)
              } else {
                const id = agentMsgId
                setViewSession((s) => s ? { ...s, msgs: s.msgs.map((m) => m.id === id ? { ...m, text: m.text + ev.text } : m) } : s)
              }
            } else if (ev.type === 'done') {
              setRunning(false)
              notifyAgentMessage(k)
              // Reload viewSession from DB — replaces optimistic bubbles with persisted state
              if (viewSession?.id) {
                try {
                  const rld = await fetch(`/api/chat/sessions/${viewSession.id}/messages`)
                  const msgs = await rld.json() as Message[]
                  const updated = { ...viewSession, msgs }
                  lastViewSessionRef.current = updated
        ...
        if ((err as { name?: string }).name !== 'AbortError') {
          if (agentMsgId === null) {
            const id = 'a' + Date.now()
            agentMsgId = id
            setViewSession((s) => s ? { ...s, msgs: [...s.msgs, { id, role: 'agent', text: 'Error: could not reach server.', at: nowTime() }] } : s)
          } else {
            const id = agentMsgId
            setViewSession((s) => s ? { ...s, msgs: s.msgs.map((m) => m.id === id ? { ...m, text: m.text + '\nError: could not reach server.' } : m) } : s)
          }
        }
      } finally {
        setRunning(false)
  ...
  return (
    ...
    <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', minHeight: 0, position: 'relative' }}>
                      ...
                      >
                        ...
                        {isUser && (
                          <div style={{ display: 'flex', alignItems: 'center', gap: 5, marginTop: 7, paddingLeft: 50, fontSize: 11.5, color: '#565d72' }}>
                            <StatusTick color={running && i === displayThread.length - 1 ? '#6a7088' : 'var(--success)'} />
                            <span>{m.at}</span>
                          </div>
                        )}
      ...
      {activeCron || activeSentry || activeLinear ? (
        <div style={{ flex: 'none', padding: '16px 22px 20px', position: 'relative', zIndex: 1 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 9, padding: '14px 16px', background: '#11151f', border: '1px solid var(--tile-border)', borderRadius: 16, color: '#6a7088', fontSize: 13 }}>
            {activeSentry ? <BellIcon size={15} color="#6a7088" /> : activeLinear ? <LinearIcon size={15} color="#6a7088" /> : <ClockIcon size={15} color="#6a7088" />}
            Read-only · {activeSentry ? 'Sentry alerts' : activeLinear ? 'Linear reports' : 'Cron output'}
          </div>
        </div>
      ) : (
      <div style={{ flex: 'none', padding: '14px 22px 18px', position: 'relative', zIndex: 1 }}>
        {/* /linear search results — floats above composer, keyboard-navigable */}
        {linearOpen && (
          <div
            style={{
              position: 'absolute', bottom: 'calc(100% - 14px)', left: 22, right: 22, marginBottom: 4,
              background: '#0c1119', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 12,
              boxShadow: '0 8px 28px rgba(0,0,0,0.55)', overflow: 'hidden', zIndex: 30,
              maxHeight: 340, overflowY: 'auto',
              animation: 'hcmdin 0.15s cubic-bezier(0.16,1,0.3,1)',
            }}
          >
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 13px 5px' }}>
              <span style={{ fontSize: 10, textTransform: 'uppercase', letterSpacing: '0.09em', color: '#565d72' }}>
                Linear Issues{linearResults.length > 0 ? ` · ${linearResults.length}` : ''}
              </span>
              <span style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                {linearResults.length > 0 && (
                  <span style={{ fontSize: 10, color: '#565d72' }}>↑↓ navigate · ↵ open</span>
                )}
                <button
                  onClick={() => setLinearOpen(false)}
                  style={{ background: 'none', border: 'none', color: '#565d72', cursor: 'pointer', fontSize: 13, lineHeight: 1, padding: 0 }}
                >✕</button>
              </span>
            </div>
            {linearLoading && (
              <div style={{ padding: '10px 13px 12px', fontSize: 13, color: '#9298ab' }}>Searching Linear…</div>
            )}
            {!linearLoading && linearError && (
              <div style={{ padding: '10px 13px 12px', fontSize: 13, color: '#f87171' }}>{linearError}</div>
            )}
            {!linearLoading && !linearError && linearResults.map((issue, i) => (
              <div
                key={issue.identifier || i}
                onMouseDown={(e) => { e.preventDefault(); openLinearIssue(i) }}
                onMouseEnter={() => setLinearIdx(i)}
                style={{
                  display: 'flex', alignItems: 'flex-start', gap: 10, padding: '9px 13px', cursor: 'pointer',
                  background: i === linearIdx ? 'rgba(255,255,255,0.07)' : 'transparent',
                  borderTop: '1px solid rgba(255,255,255,0.04)',
                }}
              >
                <span style={{ width: 8, height: 8, borderRadius: '50%', flex: 'none', marginTop: 5, background: linearPriorityColor(issue.priority) }} title={issue.priority_label} />
                <span style={{ minWidth: 0, flex: 1, display: 'flex', flexDirection: 'column', gap: 2 }}>
                  <span style={{ display: 'flex', alignItems: 'baseline', gap: 7, minWidth: 0 }}>
                    <span style={{ fontFamily: 'IBM Plex Mono, monospace', fontSize: 11.5, color: accent, fontWeight: 700, flex: 'none' }}>{issue.identifier}</span>
                    <span style={{ fontSize: 13.5, color: '#e9ebf2', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{issue.title}</span>
                  </span>
                  <span style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 11, color: '#9298ab' }}>
                    {issue.status && <span>{issue.status}</span>}
                    {issue.status && <span style={{ color: '#3a4055' }}>·</span>}
        ...
        >
          ...
          <textarea
            ...
            onKeyDown={(e) => {
              // Linear results panel takes keyboard priority when open + populated.
              if (linearOpen && linearResults.length > 0) {
                if (e.key === 'ArrowUp') {
                  e.preventDefault()
                  setLinearIdx((i) => (i - 1 + linearResults.length) % linearResults.length)
                  return
                }
                if (e.key === 'ArrowDown') {
                  e.preventDefault()
                  setLinearIdx((i) => (i + 1) % linearResults.length)
                  return
                }
                if (e.key === 'Enter' && !e.shiftKey) {
                  e.preventDefault()
                  openLinearIssue(linearIdx)
                  return
                }
                if (e.key === 'Escape') {
                  e.preventDefault()
