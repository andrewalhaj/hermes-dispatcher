...
import { ACCENT_SWATCHES, API_KEYS, LANG_OPTS, SETTINGS_TOGGLES } from '../../data/phase3'
...
// Fallback model list when /api/models is unreachable on mount.
const MODEL_FALLBACK = ['Claude Sonnet 4.6', 'Claude Opus 4', 'Claude Haiku 4']
...
export default function Settings({ accent = ACCENT, onAccentChange }: SettingsProps) {
  ...
  const [model, setModel] = useState(() => lsGet(LS_MODEL, MODEL_FALLBACK[0]))
  const [modelOpts, setModelOpts] = useState<string[]>(MODEL_FALLBACK)
  ...
  useEffect(() => {
    ...
    fetch('/api/models')
      .then((r) => r.json())
      .then((d) => {
        if (Array.isArray(d.catalog) && d.catalog.length > 0) setModelOpts(d.catalog)
      })
      .catch(() => {})
