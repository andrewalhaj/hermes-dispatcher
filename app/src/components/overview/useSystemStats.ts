 ...
 * (null — non-NVIDIA host). Pass `machine` to switch between the local host (MM01)
 * (`mini`) and the remote inference node (MS01) (`studio`); rolling buffers reset on switch
...
export function useSystemStats(machine: 'mini' | 'studio' = 'mini'): SystemStats {
  ...
  return {
    ...
    agents,
    hasAnySpike,
    hostLabel: machine === 'studio' ? 'MS01' : 'MM01',
    unreachable,
  }
}
