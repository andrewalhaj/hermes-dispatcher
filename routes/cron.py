...
Data sources (real shapes, verified 2026-06-23):
  - ~/.hermes/cron/jobs.json   ->  {"jobs": [ {id, name, schedule:{display|expr}, enabled, ...}, ... ]}
  - ~/.hermes/cron/output/<job_id>/<YYYY-MM-DD_HH-MM-SS>.md   (one file per run)
...
if not JOBS.exists():
    JOBS = Path(os.environ.get("HERMES_HOME", os.path.expanduser("~/.hermes"))) / "cron" / "jobs.json"
