...
# Remote machine (MS01) reachable over Tailscale via SSH.
STUDIO_HOST = os.environ.get("STUDIO_HOST", "")
...
# Intel iGPU (e.g. MM01 UHD 630) debugfs path for VRAM/stolen-memory readout.
...
def _network_mbps(state: dict[str, float] | None = None, total: float | None = None) -> float:
    ...
    ``state`` defaults to the local MM01 delta dict; pass ``_last_net_studio``
    (and the remote ``total`` byte count) to compute the MS01's throughput.
    """
    if state is None:
        state = _last_net
    if total is None:
...
def _gpu_stats() -> dict:
    ...
    MM01 (UHD 630). Returns ``None`` values when neither is available.
    """
    if _HAS_NVIDIA_SMI:
...
async def get_system(machine: str = "mini") -> dict:
    ...
    fetches the remote MS01's metrics over SSH (Tailscale). The running
...
def _fetch_studio_metrics() -> dict:
    """Fetch MS01 metrics over SSH (Tailscale). Blocking — run in executor.
