---
name: kanban-worker
description: "plan to delegate? workers fan out 2+ subtasks."
version: 2.0.0
...
## Autonomous subtask delegation (max_spawn_depth=2)

Workers can and should call `delegate_task` for 2+ **independent** subtasks within their
own run. No proposal needed — fan out immediately.

- 2+ independent read/analysis subtasks → `delegate_task` in parallel, synthesize results
- Sequential dependency chain → do it directly
- State-changing work → WRITE GATE still governs inside subagents

`delegate_task` is for short reasoning subtasks inside YOUR run that can execute in
parallel. `kanban_create` is for cross-agent handoffs that outlive one API loop — use it
for follow-up work assigned to another profile, not for your own parallel subtasks.
