...
hermes sessions optimize    Reclaim disk space (VACUUM + FTS merge)

### Delete all sessions (fresh slate)

There is no `--all` flag. Pipe the list into individual deletes:

```bash
hermes sessions list | grep -oP '<session_id_regex>' | while read id; do
  hermes sessions delete --yes "$id"
done
hermes sessions optimize
```

The current session (the one you're in) cannot be deleted — one session always remains.
`hermes sessions optimize` vacuums the DB after mass-deletion.
...
Edit with `hermes config edit` or `hermes config set section.key value`. **Do not use `patch` or `write_file` on `~/.hermes/config.yaml`** — Hermes blocks direct file edits to its own config and redirects you to use the CLI instead.
