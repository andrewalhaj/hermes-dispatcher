...
description: "Anti-slop frontend: landing pages, portfolios, apps."
---
...
* **DOUBLE-BEZEL NESTED ARCHITECTURE (premium card pattern):** For luxury / premium-consumer briefs where cards need physical depth: never place a card flat on the background. Nest it inside an outer shell (`ring-1 ring-black/5`, `p-1.5`, `rounded-[2rem]`) with an inner core that has its own background and an inner highlight (`shadow-[inset_0_1px_1px_rgba(255,255,255,0.15)]`). The inner core radius must be mathematically smaller than the outer shell (`rounded-[calc(2rem-0.375rem)]` for concentric curves). Apply ONLY when the design read calls for physical materiality (premium consumer, luxury, Apple-adjacent). Overkill for dashboards, public-sector, or data-dense interfaces.
