...
## Cron → Linear → Kanban Autonomous Intake

Linear issues can serve as an autonomous intake funnel: cron jobs detect issues → create Linear issues via API → webhook auto-routes to Kanban → coder fleet picks them up.
...
cron detects anomaly
  → POST /graphql (create Linear issue)
    → Linear webhook fires at /api/hooks/linear
      → handler stores to knowledge store (fire-and-forget)
      → handler dispatches Kanban card (round-robin coder assignment)
...
Reference: `~/.hermes/references/cron-linear-intake.md` — curl template with team UUID and API key.
...
- `LINEAR_API_KEY` — personal API key for issue creation
- `LINEAR_WEBHOOK_SECRET` — webhook HMAC secret (for handler auth)
