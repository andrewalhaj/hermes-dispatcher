...
# ---------------------------------------------------------------------------
# Complexity classifier — auto-upgrade Sonnet → Fable for heavy tasks
# ---------------------------------------------------------------------------

# Patterns that collectively signal a task warrants Fable 5 over Sonnet 4.6.
_COMPLEX_SIGNALS = [
    # Architecture & design work
    "refactor", "architecture", "design pattern", "design system",
    "system design", "restructure", "rebuild",
    # Multi-step / production
    "migration", "deploy", "production", "multi-file",
    "across the codebase", "end-to-end", "full system",
    # Deep analysis
    "audit", "security review", "vulnerability",
    "performance analysis", "benchmark", "optimization",
    # Heavy implementation
    "from scratch", "build a", "implement a", "new feature",
    # Long-form / comprehensive
    "comprehensive", "detailed analysis", "thorough",
    "write a report", "generate documentation",
    # Reasoning-heavy
    "debug", "diagnose", "root cause", "troubleshoot",
    "investigate",
]

_HEAVY_MODEL = "claude-opus-4-8"
...
def _classify_complexity(api_kwargs: Dict[str, Any]) -> bool:
    """Return True if the request warrants a heavy-model upgrade.

    Scans ONLY the most recent human turn — the last ``role:"user"``
    message that carries real text.  This deliberately ignores:

    * the system prompt (keyword-saturated boilerplate that scored 4+
      signals on its own, forcing an upgrade on *every* request); and
    * tool_result turns (the API encodes these as ``role:"user"`` with
      ``tool_result`` blocks and no top-level text), so a mid-agentic-loop
      tool result can no longer drop a complex task back to Sonnet.

    Uses lightweight keyword matching — fast, no network round-trip, zero
    token overhead for the classification pass.

    Only upgrades Sonnet → heavy; never downgrades an already-heavy
    request (``_maybe_upgrade_model`` gates on "sonnet" in model name).
    """
    ...
    score = sum(1 for s in _COMPLEX_SIGNALS if s in combined)
    prompt_len = len(combined)

    if score >= _COMPLEX_SCORE_THRESHOLD:
        return True
    if score >= 1 and prompt_len > _COMPLEX_LEN_THRESHOLD:
        return True
    return False


def _maybe_upgrade_model(api_kwargs: Dict[str, Any]) -> str:
    """Swap ``claude-sonnet-*`` → ``claude-fable-5`` on complex tasks.

    Returns the model name that will be used (original or upgraded).
    Idempotent — only gates on sonnet, so an already-heavy request is
    never downgraded.
    """
    current_model = str(api_kwargs.get("model") or "")
    if "sonnet" not in current_model.lower():
        return current_model

    if not _classify_complexity(api_kwargs):
        return current_model

    api_kwargs["model"] = _HEAVY_MODEL
    logger.info(
        "Complexity upgrade: %s → %s (prompt_score=%d signals)",
        current_model,
        _HEAVY_MODEL,
        getattr(api_kwargs, "__complexity_score__", 0),  # best-effort
    )
    return _HEAVY_MODEL
...
def apply_patches(anthropic_adapter_module: Any = None) -> bool:
    ...
    # ── Delegation-checkpoint guard (Option A — chain from bypass) ────────
    # Belt-and-suspenders for the Anthropic path. The primary install comes
    # from sitecustomize.py at startup (provider-independent). This call is
    # idempotent — no-ops if the guard is already armed or installed.
    try:
        import delegation_checkpoint as _dc
        _dc.apply_patches()
    except Exception as _dc_exc:
        sys.stderr.write(
            f"[delegation-checkpoint] bypass chain error (no-op): "
            f"{type(_dc_exc).__name__}: {_dc_exc}\n"
        )

    # ── Skill-review checkpoint (Option A — chain from bypass) ────────────
    # Surfaces matching skill candidates on COMPLEX tasks when no skill has
    # been loaded yet. Primary install is from sitecustomize.py at startup;
    # this chain is belt-and-suspenders for the Anthropic path. Idempotent.
    try:
        import skill_review_checkpoint as _src
        _src.apply_patches()
    except Exception as _src_exc:
        sys.stderr.write(
            f"[skill-review-checkpoint] bypass chain error (no-op): "
            f"{type(_src_exc).__name__}: {_src_exc}\n"
        )

    # ── Memory-checkpoint guard (Option A — chain from bypass) ─────────────
    # Appends an in-band nudge to the tool result on every memory write that
    # pushes a store above the warn threshold. Primary install is from
    # sitecustomize.py at startup; this chain is belt-and-suspenders for the
    # Anthropic path. Idempotent.
    try:
        import memory_checkpoint as _mc
        _mc.apply_patches()
    except Exception as _mc_exc:
        sys.stderr.write(
            f"[memory-checkpoint] bypass chain error (no-op): "
            f"{type(_mc_exc).__name__}: {_mc_exc}\n"
        )

    # ── Domain-ownership guard (Option A — chain from bypass) ──────────────
    # Nudges when a state-changing ssh/scp targets a peer-owned host/path.
    # Primary install is from sitecustomize.py at startup; this chain is
    # belt-and-suspenders for the Anthropic path. Idempotent.
    try:
        import domain_ownership_checkpoint as _doc
        _doc.apply_patches()
    except Exception as _doc_exc:
        sys.stderr.write(
            f"[domain-ownership-checkpoint] bypass chain error (no-op): "
            f"{type(_doc_exc).__name__}: {_doc_exc}\n"
        )

    # ── Write-gate guard (Option A — chain from bypass) ────────────────────
    # Hard-enforcement gate on gated terminal commands and file writes to
    # protected paths. Blocks unless armed via explicit user greenlight.
    # Primary install is from sitecustomize.py at startup; this chain is
    # belt-and-suspenders for the Anthropic path. Idempotent.
    try:
        ...
        sys.stderr.write(
            f"[write-gate] bypass chain error (no-op): "
            f"{type(_wg_exc).__name__}: {_wg_exc}\n"
        )

    # ── Kanban-phase-checkpoint (Option A — chain from bypass) ────────────────
    # Scores each user message for multi-phase complexity and injects a routing
    # nudge when score >= PHASE_SCORE_MIN. Primary install is sitecustomize.py;
    # this chain is belt-and-suspenders for the Anthropic path. Idempotent.
    try:
        import kanban_phase_checkpoint as _kpc
        _kpc.apply_patches()
    except Exception as _kpc_exc:
        sys.stderr.write(
            f"[kanban-phase-checkpoint] bypass chain error (no-op): "
            f"{type(_kpc_exc).__name__}: {_kpc_exc}\n"
        )

    # ── Delegate toolset floor (Option A — chain from bypass) ──────────────────
    # Belt-and-suspenders for the Anthropic path. Primary install is from
    # sitecustomize.py at startup. Idempotent — no-ops if already armed.
    try:
        import delegate_toolset_floor as _dtf
        _dtf.apply_patches()
    except Exception as _dtf_exc:
        sys.stderr.write(
            f"[delegate-toolset-floor] bypass chain error (no-op): "
